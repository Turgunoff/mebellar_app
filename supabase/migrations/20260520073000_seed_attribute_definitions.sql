...
-- in 20260506170142_create_categories_and_subcategories.sql. Category and
-- subcategory rows live in the DB under their O'zbek display names — the
-- WHERE clauses below use those exact strings so `supabase db reset` on a
-- fresh clone reproduces what production has.
--
-- Inserts are idempotent (ON CONFLICT DO NOTHING) so re-running this migration
-- is safe — admins can ALTER labels in Studio without the seed clobbering them.
--
...
where c.name = 'Divanlar va Kreslolar'
...
where ad.key = 'fabric_type'
  and ad.category_id = (select id from public.categories where name = 'Divanlar va Kreslolar')
...
where ad.key = 'corner_side'
  and ad.subcategory_id = (select id from public.subcategories where name = 'Burchak divanlar')
on conflict (attribute_id, value) do nothing;

-- Subcategory: Yechiladigan divanlar
insert into public.attribute_definitions
  (subcategory_id, key, label_uz, label_ru, data_type, unit, is_required, sort_order)
select s.id, 'sleeper_mechanism', 'Yotoq mexanizmi', 'Механизм раскладки', 'select', null, false, 5
from public.subcategories s
where s.name = 'Yechiladigan divanlar'
on conflict (subcategory_id, key) do nothing;
...
where ad.key = 'sleeper_mechanism'
  and ad.subcategory_id = (select id from public.subcategories where name = 'Yechiladigan divanlar')
on conflict (attribute_id, value) do nothing;

-- ===== STOLLAR VA YOZUV STOLLARI =====
insert into public.attribute_definitions
  (category_id, key, label_uz, label_ru, data_type, unit, is_required, sort_order)
select c.id, v.key, v.label_uz, v.label_ru, v.data_type, v.unit, v.is_required, v.sort_order
from public.categories c
...
where c.name = 'Stollar va Yozuv stollari'
on conflict (category_id, key) do nothing;
...
where ad.key = 'top_material'
  and ad.category_id = (select id from public.categories where name = 'Stollar va Yozuv stollari')
on conflict (attribute_id, value) do nothing;

-- ===== KARAVOTLAR VA YOTOQXONALAR =====
insert into public.attribute_definitions
  (category_id, key, label_uz, label_ru, data_type, unit, is_required, sort_order)
select c.id, v.key, v.label_uz, v.label_ru, v.data_type, v.unit, v.is_required, v.sort_order
from public.categories c
...
where c.name = 'Karavotlar va Yotoqxonalar'
on conflict (category_id, key) do nothing;
...
where ad.key = 'mattress_size'
  and ad.category_id = (select id from public.categories where name = 'Karavotlar va Yotoqxonalar')
on conflict (attribute_id, value) do nothing;
...
where c.name = 'Stullar va O''rindiqlar'
on conflict (category_id, key) do nothing;
...
where ad.key = 'seat_material'
  and ad.category_id = (select id from public.categories where name = 'Stullar va O''rindiqlar')
on conflict (attribute_id, value) do nothing;
...
where ad.key = 'bulb_type'
  and ad.category_id = (select id from public.categories where name = 'Yoritish')
on conflict (attribute_id, value) do nothing;
...
where c.name = 'Saqlash va Shkaflar'
on conflict (category_id, key) do nothing;
...
where ad.key = 'opening_type'
  and ad.subcategory_id = (select id from public.subcategories where name = 'Shkaflar')
on conflict (attribute_id, value) do nothing;
...
where c.name = 'Dekor va Aksessuarlar'
on conflict (category_id, key) do nothing;

-- ===== BOG' VA TASHQI MAYDON =====
insert into public.attribute_definitions
  (category_id, key, label_uz, label_ru, data_type, unit, is_required, sort_order)
select c.id, v.key, v.label_uz, v.label_ru, v.data_type, v.unit, v.is_required, v.sort_order
from public.categories c
...
where c.name = 'Bog'' va Tashqi maydon'
on conflict (category_id, key) do nothing;
...
where ad.key = 'outdoor_material'
  and ad.category_id = (select id from public.categories where name = 'Bog'' va Tashqi maydon')
on conflict (attribute_id, value) do nothing;
