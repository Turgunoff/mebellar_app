...
void main() {
  ...
  setUp(() {
    repo = _MockAuthRepo();
    when(
      () => repo.requestOtp(any(), appLanguage: any(named: 'appLanguage')),
    ).thenAnswer((_) async => 60);
    when(() => repo.verifyOtp(any(), any())).thenAnswer((_) async => tokens);
    when(() => repo.updateProfile(fullName: any(named: 'fullName'))).thenAnswer(
      (_) async =>
  ...
  test(
    'rejects an incomplete phone number without calling the backend',
    () async {
  ...
  test(
    'a valid phone advances Phone -> OTP and requests the code in E.164',
    () async {
      final c = makeController();
      ...
      verify(
        () => repo.requestOtp(
          '+998901234567',
          appLanguage: any(named: 'appLanguage'),
        ),
      ).called(1);
    },
  );
  ...
  test(
    'verifying an OTP for a profileless user advances OTP -> Profile',
    () async {
  ...
  test(
    'verifying an OTP for a user who already has a profile completes',
    () async {
      when(
        () => repo.fetchMe(),
