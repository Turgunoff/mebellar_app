...
### 2.8 To'lovlar (Payme live · Click link-only) + ichki escrow
...
- **Deferred order pay:** checkout `payment_method` saqlaydi; seller fee + customer approve → `awaiting_payment`; `POST /orders/{id}/pay/payme` → `checkout.paycom.uz` deep-link; Perform → `paid` + `confirmed` + `stamp_order_deadline`.
- **Strict `awaiting_payment` gate:** CheckPerform / Create **faqat** `status == awaiting_payment` **va** `payment_status == unpaid` bo'lsa qabul qiladi; aks holda Payme `-31050`. Link mint (`POST /orders/{id}/pay/payme`) xuddi shu qoida.
- **`CancelTransaction` after perform (refund):** order → bir xil DB tranzaksiyasida `payment_status=refunded` + **`status=cancelled`** (`cancel_reason_code=payment_refunded`, `is_delayed=false`) — lifecycle unwind. `delivered` → `-31007` (bekor qilib bo'lmaydi).
...
- **Ichki escrow (Payme Safe/Split EMAS):** platform marketplace Safe Trade API ishlatmaydi. Online to'lov → Woody ushlab turadi → `delivered` da `settle_wallet_on_delivery`:
  - COD → `commission` debet (`wallet_tx_commission_once_idx`).
  - Online → `order_income` kredit = gross − commission (`wallet_tx_order_income_once_idx`, migration 0088).
- **Payout:** `POST /seller/wallet/withdraw` → `wallet_withdrawals` + `withdrawal_hold`; admin `PATCH /admin/wallets/withdrawals/{id}` approve (hold saqlanadi) / reject (`withdrawal_refund`).
...
- Bildirishnomalar: dispatcher avval inbox qatorini claim qiladi, keyin (ixtiyoriy) FCM, so'ng broker'ga publish — FCM o'chirilganda ham WS inbox yangilanadi.

### 2.6 FCM klient abstraksiyasi + push preferenslari

- `FcmClient` Protocol; ikki implementatsiya: `NoOpFcmClient` (dev/CI default) va `FirebaseFcmClient` (`firebase-admin`).
- `send_to_tokens()` — `asyncio.to_thread()` orqali 500/batch chunk; yaroqsiz tokenlar (`UnregisteredError`/`SenderIdMismatchError`) tozalash uchun qaytariladi. `send_to_topic()` — `news` fan-out (promo).
- **`profiles.order_push_enabled`:** `NotificationDispatcher` ORDER-turdagi shaxsiy pushlarda FCM ni o'tkazib yuboradi (inbox + realtime saqlanadi). `data.send_push=false` admin broadcast uchun xuddi shu "inbox-only" shakl.
- **`profiles.promo_push_enabled`:** Flutter tomonda FCM topic `news` subscribe/unsubscribe; backend faqat preferensni saqlaydi (`PATCH /me`).
...
Yetti box (`CoreBoxes` record): `settings, cache, pendingRoute, onboardingDraft, favorites, cart, newsReads`. `newsReads` — anonim foydalanuvchilar uchun o'qilgan yangilik id'lari (`Set<String>`). Settings box kalitlari: `analytics_collection_enabled`, `promo_push_enabled`, `order_push_enabled` (barchasi default `true`).

### 3.3b Hybrid inbox + privacy

- **`NotificationsCubit`:** mehmonlar `GET /catalog/news` (auth yo'q); shaxsiy `GET /notifications` faqat login'dan keyin. Buyurtma / shaxsiy tab'lar login CTA ko'rsatadi.
- **Analytics privacy:** `applyAnalyticsCollectionEnabled` — Firebase Analytics + Crashlytics (`enabled && !kDebugMode`) + Meta privacy flag. Cold start: Firebase init'dan keyin collection mute → Hive ochilgach preferens qo'llaniladi → `runApp`.
- **Promo push:** Settings toggle → FCM `news` topic subscribe/unsubscribe + `PATCH /me`.
...
| Domen | Jadvallar |
|---|---|
| Identitet | `profiles` (phone, app_metadata_role, app_metadata_permissions, is_seller_pending, deleted_at, blocked_at, **promo_push_enabled**, **order_push_enabled**), `otp_codes`, `refresh_tokens` |
...
| Pul | `subscription_plans`, `subscriptions`, `subscription_history`, `subscription_receipts`, `wallet_transactions` (… + **order_income / withdrawal_hold / withdrawal_refund**), `wallet_topups`, `wallet_deposits`, **`wallet_withdrawals`**, `ar_token_purchases` |
| Aloqa | `chats` (order_id UNIQUE), `chat_messages`, `notifications`, `device_tokens`, `banners`, `news` |

**Asosiy indekslar:** `profiles_phone_unique_idx` (partial, live), `sellers_debt_grace_sweep_idx`, `wallet_tx_commission_once_idx` + `wallet_tx_order_income_once_idx` (idempotentlik), `products_status_idx`, `orders_status_idx`, `chats_order_id_idx`.

**Migration'lar:** Alembic, baseline `0001_baseline_schema.sql` (Supabase eksportidan, RLS/auth.users/pg_net olib tashlangan) + keyingi revisionlar. Joriy head misoli: **`0089_push_preferences`** (0088 escrow withdrawals'dan keyin). `AUTO_MIGRATE` prod'da o'chiq; deploy `woody migrate` ishlatadi.
