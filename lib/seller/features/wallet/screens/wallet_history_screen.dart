...
class _WalletHistoryScreenState extends State<WalletHistoryScreen> {
  ...
  Widget _buildBody(SellerColors c) {
    ...
    final withdrawals = _withdrawals ?? const [];
    final transactions = _transactions ?? const [];
    if (topUps.isEmpty && withdrawals.isEmpty && transactions.isEmpty) {
      ...
      child: ListView(
        ...
        children: [
          if (withdrawals.isNotEmpty) ...[
            _SectionTitle(label: tr('seller.wallet_withdrawals_title')),
            const SizedBox(height: 10),
            for (var i = 0; i < withdrawals.length; i++) ...[
              if (i > 0) const SizedBox(height: 10),
              _WithdrawalTile(withdrawal: withdrawals[i]),
            ],
...
class _WithdrawalTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final c = SellerColors.of(context);
    final palette = _statusPalette(c, withdrawal.status);
    return Container(
      decoration: BoxDecoration(
        color: c.surface,
        borderRadius: BorderRadius.circular(16),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Container(
                ...
                child: Icon(Iconsax.send_2, color: c.primary, size: 20),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _when.format(withdrawal.createdAt.toLocal()),
                      style: TextStyle(
                        fontFamily: AppFonts.seller,
                        fontSize: 12,
                        fontWeight: FontWeight.w500,
          ...
          Text(
            '${_uzs.format(withdrawal.amount)} ${tr('seller.wallet_currency_som')}',
            style: TextStyle(
              fontFamily: AppFonts.seller,
              fontSize: 20,
              fontWeight: FontWeight.w800,
          ...
          if (withdrawal.rejectionReason case final reason?
  ...
  static String _statusLabel(String status) => switch (status) {
    'approved' => tr('seller.wallet_withdrawal_status_approved'),
    'rejected' => tr('seller.wallet_withdrawal_status_rejected'),
    _ => tr('seller.wallet_withdrawal_status_pending'),
  };
